...
export default function AnalyzeWebsite() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Get ObjectID for authentication (optional for analysis - NEW OBJECTID SYSTEM)
      ...
      if (userObjectId) {
        headers['Authorization'] = `Bearer ${userObjectId}`; // ObjectID instead of JWT
        console.log('🔍 ANALYZE: Using ObjectID for authentication:', userObjectId.substring(0, 8) + '...');
      } else {
        ...
        {analysisResult && (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20">
                <h3 className="text-xl font-bold text-white mb-4">⚠️ Areas for Improvement</h3>
                <ul className="space-y-2">
              ...
              <div className="space-y-4">
                {analysisResult.insights.recommendations.map((rec, index) => (
                  <div key={index} className="border border-white/20 rounded-lg p-4">
                    ...
                    <p className="text-sm text-blue-300">Impact: {rec.impact}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20">
              <h3 className="text-xl font-bold text-white mb-4">💰 Affiliate Optimization</h3>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                  <div className="mb-4">
                    <h5 className="font-semibold text-white mb-2">Conversion Potential:</h5>
                    <p className="text-gray-300">{analysisResult.insights.affiliateOptimization.conversionPotential}</p>
                  </div>
